...
const CompanyDashboard = () => {
    ...
    const fetchVouchers = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `${baseUrl}/api/users/companyDashboard?userId=${user?._id}`,
          {
            headers: {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
      <CardDataStats
        title="Bids Remaining"
        // total="$3.456K"
        total={data?.remainingBids}
        // rate="0.43%"
        levelUp
      >
        <svg
      ...
      <CardDataStats title="Total Likes" total={`${data?.totalLikes}`} levelUp>
        <svg
          className="fill-primary dark:fill-white"
          width="22"
          height="16"
